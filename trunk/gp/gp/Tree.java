...
public class Tree {
	 ...
	 * a height of zero.
	 */
	public int getHeight(int newNumberOfNodes) {
	...
	private void printTree(Node newNode, int newCount) throws Exception {
		// if(this.getNodes().size() < newCount-1){
		// for(int m = 0; m < getNodes().size(); m++){
		// getNodes().get(m).printNodeInfo();
		// }
		// throw new Exception ("Something is wrong with the tree " +
		// this.getNodes().size() +" "+ newCount);
		// }
		Node leftChild = null;
